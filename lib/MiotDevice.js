...
class MiotDevice extends EventEmitter {
  ...
  supportsLedBrightness() {
    return this.capabilities[Capabilities.LED_CONTROL_BRIGHTNESS] || false; // if indicator light can be controlled like a light bulb with 0 to 100% percent values
  }
  ...
  supportsUseTimeReporting() {
    return this.properties[Properties.USE_TIME] !== undefined; // whether the device returns use time
  }

  useTimeUnit() {
    return this.capabilities[Capabilities.USE_TIME_UNIT] || Constants.TIME_UNIT_MINUTES;
  }
  ...
  getShutdownTimer() {
    ...
    if (this.powerOffTimerUnit() === Constants.TIME_UNIT_HOURS) {
      return value * 60;
    } else if (this.powerOffTimerUnit() === Constants.TIME_UNIT_SECONDS) {
  ...
  getUseTime() {
    ...
    if (this.useTimeUnit() === Constants.TIME_UNIT_HOURS) {
      return useTime * 60;
    } else if (this.useTimeUnit() === Constants.TIME_UNIT_SECONDS) {
