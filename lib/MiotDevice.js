...
class MiotDevice extends EventEmitter {
  ...
  supportsPowerOffTimer() {
    return this.properties[Properties.POWER_OFF_TIME] !== undefined;
  }
  ...
  supportsBuzzerControl() {
    return this.properties[Properties.ALARM] !== undefined;
  }

  // led
  supportsLedControl() {
    return this.properties[Properties.LED] !== undefined;
  }

  ledControlRange() {
    return this.supportsLedControl() ? this.properties[Properties.LED].getValueRange() : [];
  }
  ...
  setPropertyValue(propName, value) {
    ...
    if (prop) {
      ...
      if (propRange != null && propRange.length > 1) {
        ...
        if (value > high) {
          this.logger.debug(`Trying to set ${propName} property with an out of range value: ${value}. Adjusting value to max: ${high}`);
          value = high;
        } else if (value < low) {
          this.logger.debug(`Trying to set ${propName} property with an out of range value: ${value}. Adjusting value to min: ${low}`);
          value = low;
        }
      }
  ...
  updatePropertyValue(result, propName, returnObj) {
    if (returnObj.code === 0) {
      this.properties[propName].setValue(returnObj.value);
      ...
      if (this.isConnected()) {
        ...
        return this.miioDevice.call('get_properties', [propDef])
          .then(result => {
            ...
            const obj = {};
            this.updatePropertyValue(obj, prop.getName(), result[0]);
            this.emit(Events.DEVICE_DEVICE_MANUAL_PROPERTIES_UPDATE, result);
            return obj;
          }).catch(err => {
