...
class MiotDevice extends EventEmitter {
  ...
  async setProperty(propName, value) {
    if (this.isConnected()) {
      ...
      this.logger.deepDebug(`Set ${propName} property request! RAW: ${JSON.stringify(propDef)}`);
  ...
  async requestProperty(propName) {
    if (this.isConnected()) {
      ...
      this.logger.deepDebug(`Request ${propName} property! RAW: ${JSON.stringify(propDef)}`);
