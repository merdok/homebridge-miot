...
class MiotSpecClassGenerator extends EventEmitter {
  ...
  async generate() {
    ...
    if (this.miotSpecUrl) {
      ...
      const specResult = await MiotSpecFetcher.fetchMiotSpecFromUrl(this.miotSpecUrl);
      const {
        type,
        description,
        services,
  ...
  async _generateServices(services) {
    ...
    if (services) {
      services.forEach(async serviceSpec => {
        if (!serviceSpec.type.includes('device-information')) { // skip services from the device information, we do not need that
