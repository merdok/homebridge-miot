...
class MiotSpecFetcher {
  ...
  async fetchMiotSpecFromUrl(specUrl) {
    if (specUrl) {
      ...
      const result = await this._processMiotFetchResult(res);
      return result;
    } else {
  ...
  async _processMiotFetchResult(data) {
    const {
      type,
      description,
      services
