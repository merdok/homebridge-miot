...
class MiotProperty extends EventEmitter {
  constructor(name, siid, piid, format, access, unit, valueRange, valueList, device) {
    super();
  ...
  updateInternalValue(newVal) {
    if (this.getValue() !== newVal && this.isWriteOnly() === false) {
      this.value = newVal;
      this.emit(Events.PROP_VALUE_CHANGED);
    }
  }
  ...
  async setValue(value) {
    if (this.device && this.device.setPropertyValue) {
      return this.device.setPropertyValue(this.getName(), value);
    }
  }
  ...
  adjustValueToPropRange(value) {
    ...
    if (valueRange != null && valueRange.length > 1 && value != undefined) {
