...
class MiCloud {
  ...
  async login(username, password) {
    this.logger.debug(`Login in to MiCloud with username ${username}`);
    if (this.isLoggedIn()) {
  ...
  async request(path, data, country = 'cn') {
    ...
    if (!this.countries.includes(country)) {
      throw new Error(`The country ${country} is not supported, list of supported countries is ${this.countries.join(', ')}`);
    }
    ...
    // result code 3 means auth error?
    if (res && res.code === 3) {
      this.logger.debug(`Auth error! Status: ${res.statusText}! Trying to reconnect!`);
      this.refreshServiceToken();
      throw new Error(`Auth error! Trying to refresh service token!`);
    }
  ...
  async miotGetProps(params, options = {}) {
    const req = {
      params
    };
    const data = await this.request(`/miotspec/prop/get`, req, options.country);
    return data.result;
  }

  async miotSetProps(params, options = {}) {
    const req = {
      params
    };
    const data = await this.request(`/miotspec/prop/set`, req, options.country);
    return data.result;
  }

  async miotAction(params, options = {}) {
    const req = {
      params
    };
    const data = await this.request(`/miotspec/action`, req, options.country);
    return data.result;
  }
  ...
  async _loginStep1() {
    ...
    this.logger.deepDebug(`MiCloud - Login step 1: ${statusText} - ${content}`);
  ...
  async _loginStep2(username, password, sign) {
    ...
    this.logger.deepDebug(`MiCloud - Login step 2: ${statusText} - ${content}`);
  ...
  async _loginStep3(location) {
    ...
    this.logger.deepDebug(`MiCloud - Login step 3: ${statusText} - ${content}`);
