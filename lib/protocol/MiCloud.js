...
class MiCloud {
  ...
  async loginTwoFa(verifyUrl, ticket) {
    ...
    for (let i = 0; i < 10; i++) {
      this.logger.deepDebug(`\n(MiCloud) [DEBUG] Redirect loop ${i}: Fetching from URL: ${locationToFollow}`);
      ...
      this.logger.deepDebug(`(MiCloud) [DEBUG] Redirect loop ${i}: Response headers:\n${JSON.stringify(Object.fromEntries(response.headers.entries()), null, 2)}`);
  ...
  async _verifyTicket(verifyUrl, ticket) {
    ...
    const identitySession = listRes.headers.get('set-cookie');
    if (!identitySession) {
      throw new Error('Invalid response. Could not find identity session in header');
    }
