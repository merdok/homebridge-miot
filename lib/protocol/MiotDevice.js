...
const LUMI_MOTION_MODELS = ['lumi.sensor_motion.v2', 'lumi.sensor_motion.aq2'];
...
class MiotDevice extends EventEmitter {
  ...
  _updatePropertyValueFromDevice(result, propName, response) {
    if (this._isResponseValid(response)) {
      if (LUMI_MOTION_MODELS.includes(this.getModel())) {
        this._updatePropertyValueBasedOnUpdateTimeFromDevice(response);
      }
  ...
  // updates the property value based on the updateTime retrieved from the device
  _updatePropertyValueBasedOnUpdateTimeFromDevice(response) {
    let currentTime = Math.floor(Date.now() / 1000); // convert to seconds
    let motionTime = response.updateTime; // updateTime is in seconds
    let motionTimeout = 60; // lumi motion has 60 seconds default timeout, also in seconds
    if (currentTime - motionTime <= motionTimeout) {
      response.value = true;
    } else {
