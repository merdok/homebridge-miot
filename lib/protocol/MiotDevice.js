...
const MiCloudErrorCodes = require("../utils/MiCloudErrorCodes");
const DeviceNotOnlineError = require("../utils/errors/DeviceNotOnlineError");
const MissingDeviceIdError = require("../utils/errors/MissingDeviceIdError");
const DeviceNotFoundError = require("../utils/errors/DeviceNotFoundError");
const UnknownDeviceModelError = require("../utils/errors/UnknownDeviceModelError");
const MissingMiCloudCredentialsError = require("../utils/errors/MissingMiCloudCredentialsError");
...
class MiotDevice extends EventEmitter {
  ...
  async _connectToCloudDevice() {
    if (!this.isConnectedToCloud()) {
      if (this._canUseMiCloud()) {
        ...
        throw new MissingMiCloudCredentialsError();
      }
    }
  }
  ...
  async _fetchCloudDeviceInfoDevice() {
    ...
    if (result != undefined) {
      ...
      throw new DeviceNotFoundError(this.getDeviceId(), this._getMiCloudCountry());
    }
  }
  ...
  async _handleConnectionError(err, actionName, timeoutFunction) {
    if (err instanceof MissingDeviceIdError || err instanceof DeviceNotFoundError || err instanceof UnknownDeviceModelError || err instanceof MissingMiCloudCredentialsError) { // non recoverable errors, do not try to reconnect
      this.logger.error(err);
      this.logger.error(`Stoping ${ actionName }! Missing or invalid required information!`);
    } else {
  ...
  async _identfiyDeviceInternal(foundDeviceModel) {
    if (foundDeviceModel) {
      ...
      throw new UnknownDeviceModelError();
    }
  }
  ...
  _validateDid() {
    ...
    if (!this.getDeviceId()) {
      if (this.shouldUseMiCloud()) {
        throw new MissingDeviceIdError(this.name);
      } else {
    ...
    this.updateDevicePropertiesInterval = setInterval(async () => {
      try {
        ...
        if (this.updateDevicePropertiesInterval && this.pollRetries >= MAX_POLL_RETRIES) {
          ...
          this.pollRetries = 0;
          if (err instanceof DeviceNotOnlineError) { // no need to disconnect when device is not online, just retry after some time
  ...
  _isResponseValid(response) {
    ...
    if (response && MiCloudUtils.compareErrorCode(response.code, MiCloudErrorCodes.DEVICE_OFFLINE)) {
      throw new DeviceNotOnlineError(this.getDeviceId());
      return false;
    }
