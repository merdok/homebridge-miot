...
class MiotDevice extends EventEmitter {
  ...
  async _connectToCloudDevice() {
    if (!this.isConnectedToCloud()) {
      if (this._canUseMiCloud()) {
        ...
        let needsLogin = true;
        if (this._getMiCloudUseCashedSession()) {
          if (this._getCashedMiCloudSession()) {
            this.logger.info(`Using cashed session for MiCloud connection! Session login time: ${this._getCashedMiCloudSession().loggedInAt}`);
            this.miCloud.setServiceToken(this._getCashedMiCloudSession());
            needsLogin = false;
          } else {
            if (this._hasMiCloudCredentials()) {
              this.logger.info(`No cashed MiCloud session found! Will try to login using the provided credentials...`);
              needsLogin = true;
            } else {
              this.logger.info(`No cashed MiCloud session found! Cannot fallback to login as credentials are missing...`);
  ...
  _getCashedMiCloudSession() {
    if (this.miCloudConfig) {
      return this.miCloudConfig.cashedSession;
    }
    return null;
  }

  _canUseMiCloud() {
    return this.miCloudConfig && (this._hasMiCloudCredentials() || this._getMiCloudUseCashedSession());
  }
  ...
  _getMiCloudUseCashedSession() {
    return this._getMiCloudConfigProp('useCashedSession');
  }
