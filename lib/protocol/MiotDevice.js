...
class MiotDevice extends EventEmitter {
  ...
  async requestAllProperties() {
    if (this.isConnected()) {
      // TODO: skip only writable properties in the prop update?
      //var readableProps = Object.keys(this.properties).filter(key => this.properties[key].isReadable());
      //let props = readableProps.map(prop => prop.getReadProtocolObjForDid(this.deviceId));
      let props = Object.keys(this.properties).map(key => this.properties[key].getReadProtocolObjForDid(this.deviceId));
      let propKeys = Object.keys(this.properties);
      return this.miioDevice.call(COMMAND_GET, props)
        .then(result => {
          const obj = {};
