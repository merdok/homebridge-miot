...
class MiotDevice extends EventEmitter {
  constructor(ip, token, deviceId, model, name, logger) {
    ...
    this.allPropRequestChunkSize = ALL_PROP_REQUEST_CHUNK_SIZE;
  ...
  getAllPropRequestChunkSize() {
    return this.allPropRequestChunkSize || ALL_PROP_REQUEST_CHUNK_SIZE;
  }

  setAllPropRequestChunkSize(chunkSize = ALL_PROP_REQUEST_CHUNK_SIZE) {
    if (chunkSize < 1) {
      chunkSize = 1;
    }
    if (chunkSize > ALL_PROP_REQUEST_CHUNK_SIZE) {
      chunkSize = ALL_PROP_REQUEST_CHUNK_SIZE;
    }
