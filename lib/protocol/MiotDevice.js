...
class MiotDevice extends EventEmitter {
  ...
  async _connectToMiCloud() {
    if (this.miCloudConfig && this.miCloudConfig.username && this.miCloudConfig.password) {
      if (!this.miCloud) {
        let requestTimeout = parseInt(this.miCloudConfig.timeout) || 5000;
        requestTimeout = requestTimeout > this.pollingInterval ? this.pollingInterval : requestTimeout; // make sure we do not exceed pollin interval
