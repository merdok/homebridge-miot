...
class MiotDevice extends EventEmitter {
  ...
  async fetchMiotSpec(forceFetch = false) {
    if (!this.getMiotSpec() || forceFetch) {
      ...
      const result = await MiotSpecFetcher.fetchMiotSpecByModel(this.getModel());
  ...
  async fetchMiotSpecFromUrl(specUrl, forceFetch = false) {
    if (!this.getMiotSpec() || forceFetch) {
      ...
      const result = await MiotSpecFetcher.fetchMiotSpecFromUrl(specUrl);
      this._processMiotSpecFetchResult(result);
    }
    return this.getMiotSpec();
  }
  ...
  async _deviceFound(isInital, fullSetup) {
    if (!this.isConnected()) {
      ...
      try {
        ...
        this.logger.info(`[Miot Device] Connected to device: ${foundDeviceModel}`);
  ...
  _disconnect() {
    ...
    this.logger.debug('[Miot Device] Device diconnected!');
  ...
  async _connectToMiCloud() {
    if (this._canUseMiCloud()) {
      ...
      if (!this.miCloud.isLoggedIn()) {
        await this.miCloud.login(this._getMiCloudUsername(), this._getMiCloudPassword(), this._getMiCloudCountry());
    ...
    if (this.getMiotSpec()) {
      ...
      if (services) {
        services.forEach((serviceSpec) => {
          if (!serviceSpec.type.includes('device-information')) { // skip device information service
      ...
      if (properties) {
        Object.keys(properties).forEach(key => {
          ...
          if (!key.includes('device-information')) { // skip properties from the device information, we do not need that
  ...
  createService(siid, type, description) {
    if (!siid) {
      this.logger.warn(`Missing siid! Cannot create service! Type: ${type} Description: ${description}`);
      return;
    }
  ...
  hasActionById(actionId) {
    return this.getActionyById(actionId) !== null;
  }
  ...
  getActionyById(actionId) {
    if (actionId && actionId.includes('.')) {
      ...
      const tmpService = this.getAllServices().find(service => service.getId() == aiid);
      if (tmpService) {
  ...
  addEvent(name, siid, eiid, type, description, argumentsDef) {
    ...
    if (!siid || !eiid) {
      this.logger.warn(`Missing siid or eiid for ${name} event! Cannot create!`);
      return;
    }
  ...
  getEventById(eventId) {
    if (eventId && eventId.includes('.')) {
      ...
      const tmpService = this.getAllServices().find(service => service.getId() == eiid);
      if (tmpService) {
    ...
    if (prop) {
      if (this.isConnected()) {
        if (prop.isReadable()) {
          ...
          return this._createErrorPromise(`Cannot update property ${prop.getName()}! Property is write only!`);
        }
      } else {
        return this._createErrorPromise(`Cannot update property ${prop.getName()}! Device not connected!`);
      }
    } else {
      return this._createErrorPromise(`Missing property! Cannot execute read request!`);
    }
  }
  ...
  async setProperty(prop, value) {
    if (prop) {
      if (this.isConnected()) {
        ...
        return this._createErrorPromise(`Cannot set property ${prop.getName()} to value ${value}! Device not connected!`);
      }
    } else {
      return this._createErrorPromise(`Missing property! Cannot set the value!`);
    }
  }
  ...
  async executeAction(action, paramValues = []) {
    if (action) {
      if (this.isConnected()) {
        ...
        return this._createErrorPromise(`Cannot execute action ${action.getName()} with params ${paramValues}! Device not connected!`);
      }
    } else {
      return this._createErrorPromise(`Missing action! Cannot execute action request!`);
    }
  }
