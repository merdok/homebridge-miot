...
class MiotDevice extends EventEmitter {
  ...
  async connectToMiCloud() {
    if (this.miCloudConfig && this.miCloudConfig.username && this.miCloudConfig.password) {
      if (!this.miCloud) {
        this.miCloud = new MiCloud(this.logger);
        await this.miCloud.login(this.miCloudConfig.username, this.miCloudConfig.password);
        this.logger.info(`Successfully connected to MiCloud!`);
      }
    } else if (this.requiresMiCloud()) {
      throw new Error(`The device requires a MiCloud connection! In order to control the device please specify a MiCloud username and password!`);
    }
  }
  ...
  async setupDevice(newMiioDevice) {
    ...
    this.emit(Events.DEVICE_SETUP_FINISHED, this);
  }
