...
class MiotDevice extends EventEmitter {
  ...
  async fireAction(actionName, params = []) {
  ...
  async sendAction(action, params = []) {
    if (action) {
      if (this.isConnected()) {
        let actionDef = action.getProtocolAction(this.deviceId, params);
        ...
        return this.miioDevice.call(COMMAND_ACTION, actionDef).then(result => {
          this.logger.debug(`Successfully executed action ${action.getName()} with params ${params}! Result: ${JSON.stringify(result)}`);
          this.emit(Events.DEVICE_ACTION_EXECUTED, action);
          this.parseActionResult(action, result);
          return action;
        }).catch(err => {
          this.logger.debug(`Error while executing action ${action.getName()} with params ${params}! ${err}`);
        });
      } else {
        return this.createErrorPromise(`Cannot execute action ${action.getName()} with params ${params}! Device not connected!`);
      }
    } else {
