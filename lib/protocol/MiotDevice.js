...
class MiotDevice extends EventEmitter {
  ...
  addProperty(name, siid, piid, type, description, format, access, unit, valueRange, valueList) {
    ...
    if (!siid) {
      this.logger.warn(`Missing siid for ${name} property! Cannot create!`);
      return;
    }

    if (!this._isValidMiotId(piid)) {
      this.logger.warn(`Missing piid for ${name} property! Cannot create!`);
      return;
    }
  ...
  addAction(name, siid, aiid, type, description, inDef) {
    ...
    if (!siid) {
      this.logger.warn(`Missing siid for ${name} action! Cannot create!`);
      return;
    }

    if (!this._isValidMiotId(aiid)) {
      this.logger.warn(`Missing aiid for ${name} action! Cannot create!`);
      return;
    }
  ...
  addEvent(name, siid, eiid, type, description, argumentsDef) {
    ...
    if (!siid) {
      this.logger.warn(`Missing siid for ${name} event! Cannot create!`);
      return;
    }

    if (!this._isValidMiotId(eiid)) {
      this.logger.warn(`Missing eiid for ${name} event! Cannot create!`);
      return;
    }
