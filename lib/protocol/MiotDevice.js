...
class MiotDevice extends EventEmitter {
  ...
  initDevice() {
    ...
    this.logger.debug(`Device actions: ${JSON.stringify(this.actions, null, 2)}`);
  ...
  addCapability(capName, value) {
    this.capabilities[capName] = value;
  }

  hasCapability(capName) {
    return this.capabilities[capName] !== undefined;
  }
  ...
  addAction(name, siid, aiid, inDef) {
    ...
    let newActions = new MiotAction(name, siid, aiid, inDef);
  ...
  async fireAction(actionName, in = []) {
    let action = this.getAction(actionName);
    if (action) {
      return this.sendAction(prop, value);
    }
  }
  ...
  // actions
  async sendAction(action, in = []) {
    if (action) {
      if (this.isConnected()) {
        let actionDef = action.getProtocolAction(this.deviceId, in );
        ...
        return this.miioDevice.call(COMMAND_ACTION, actionDef).then(result => {
          this.logger.debug(`Successfully executed action ${action.getName()} with params ${in}! Result: ${JSON.stringify(result)}`);
          this.emit(Events.DEVICE_ACTION_EXECUTED, action);
          this.parseActionResult(action, result);
          return action;
        }).catch(err => {
          this.logger.debug(`Error while executing action ${action.getName()} with params ${in}! ${err}`);
        });
      } else {
        return this.createErrorPromise(`Cannot execute action ${action.getName()} with params ${in}! Device not connected!`);
      }
    } else {
  ...
  parseActionResult(action, returnObj) {
    if (returnObj.code === 0) {
