...
class MiioProtocol extends EventEmitter {
  ...
  async send(address, method, params = [], options = {}) {
    this.logger.debug(`(Protocol) Call ${address}: ${method} - ${JSON.stringify(params)} - ${JSON.stringify(options)}`);
    const request = {
      method,
      params,
    };
    const device = this.getDevice(address);
    let requestTimeout = options.timeout && options.timeout >= 0 ? options.timeout : this.timeout;
    ...
    return new Promise((resolve, reject) => {
      ...
      let retriesLeft = options.retries && options.retries >= 0 ? options.retries : this.retries;
      const retry = () => {
        if (resolved) return;
        if (retriesLeft-- > 0) {
  ...
  setRequestTimeout(timeout = DEFAULT_TIMEOUT) {
    if (!timeout || timeout < 1000) {
      timeout = 1000; // make sure we stay above 1000ms since anything less does not make sense
    }
