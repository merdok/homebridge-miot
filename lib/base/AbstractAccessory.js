...
class AbstractAccessory {
  ...
  createStatefulSwitch(name, id, getterFn, setterFn, nameIsExplicit = false) {
    let newStatefulSwitch = new Service.Switch(this.sanitizeName(name, nameIsExplicit), id);
    this.setServiceConfiguredName(newStatefulSwitch, name, nameIsExplicit);
    newStatefulSwitch
  ...
  createStatlessSwitch(name, id, setterFn, nameIsExplicit = false) {
    let newStatelessSwitch = new Service.Switch(this.sanitizeName(name, nameIsExplicit), id);
    this.setServiceConfiguredName(newStatelessSwitch, name, nameIsExplicit);
    newStatelessSwitch
      .getCharacteristic(Characteristic.On)
  ...
  createWrapper(wrapperClass, wrapperName, prop, nameIsExplicit = false) {
    if (wrapperClass) {
      if (prop) {
        let newWrapper = new wrapperClass(wrapperName, prop, this.getDevice(), this.getAccessory(), this.api, this.logger);
        if (newWrapper) {
          newWrapper.setNameIsExplicit(nameIsExplicit || wrapperName === this.getName());
          return newWrapper;
        } else {
  ...
  addPropValueListWrapper(wrapperName, prop, linkedProp, nameIsExplicit = false) {
    const newValListWrapper = this.createWrapper(PropValueListWrapper, wrapperName, prop, nameIsExplicit);
  ...
  addPropWrapper(wrapperName, prop, linkedProp, fixedValue, linkedPropFixedValue, configuration, nameIsExplicit = false) {
    const newPropWrapper = this.createWrapper(PropertyWrapper, wrapperName, prop, nameIsExplicit);
    if (newPropWrapper) {
  ...
  addPropMonitorWrapper(wrapperName, prop, linkedProp, fixedValue, linkedPropFixedValue, fixedValueOperator, nameIsExplicit = false) {
    const newPropMonitorWrapper = this.createWrapper(PropertyMonitorWrapper, wrapperName, prop, nameIsExplicit);
  ...
  addOffDelayWrapper(wrapperName, prop, linkedProp, nameIsExplicit = false) {
    const newOffDelayWrapper = this.createWrapper(OffDelayWrapper, wrapperName, prop, nameIsExplicit);
  ...
  localizeName(name, nameIsExplicit = false) {
    if (!this.getDevice() || !this.getDevice().resolveHomeKitName) return name;
    return this.getDevice().resolveHomeKitName(name, {explicit: nameIsExplicit || name === this.getName()});
  }

  sanitizeName(name, nameIsExplicit = false) {
    return HomeKitUtils.sanitizeHomeKitName(this.localizeName(name, nameIsExplicit));
  }

  setServiceConfiguredName(service, name, nameIsExplicit = false) {
    if (service) {
      service.addOptionalCharacteristic(Characteristic.ConfiguredName);
      service.setCharacteristic(Characteristic.ConfiguredName, this.sanitizeName(name, nameIsExplicit));
    }
  }
