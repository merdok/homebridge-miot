...
class BaseAccessory {
  constructor(name, device, uuid, config, api, logger) {
    ...
    if (this.accessories && this.accessories.length > 0) {
      ...
      // setup customization services, should only be implemented in GenericAccessory
  ...
  addPropWrapper(wrapperName, prop, linkedProp, fixedValue, linkedPropFixedValue, configuration) {
    return this.createAndAddWrapper(PropertyWrapper, wrapperName, prop, linkedProp, fixedValue, linkedPropFixedValue, configuration);
  }
