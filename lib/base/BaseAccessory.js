...
class BaseAccessory {
  ...
  prepareModeControlServices() {
    if (this.getDevice().supportsModes()) {
      this.addPropValueListService('Mode', this.getDevice().modeProperty(), this.getDevice().powerProperty());
    }
  }

  prepareFanLevelControlServices() {
    if (this.getDevice().supportsFanLevels()) {
      this.addPropValueListService('Fan Level', this.getDevice().fanLevelProperty(), this.getDevice().powerProperty());
    }
  }
  ...
  prepareActionButtonServices(actionButtonsUserConfig) {
    if (this.getDevice().hasActions()) {
      ...
      let actionKeys = Object.keys(this.getDevice().getAllActions());

      // if user specified an array of keys then filter them to those which actaully exist on the device
      if (actionButtonsUserConfig && Array.isArray(actionButtonsUserConfig)) {
        let userActionKeys = actionButtonsUserConfig;
        actionKeys = userActionKeys.filter(tmpActionKey => actionKeys.includes(tmpActionKey)); // get only the action keys which actaully exist!
      }

      actionKeys.forEach((actionKey, i) => {
        let actionName = this.getDevice().getActionFriendlyName(actionKey);
        let actionId = actionKey + 'ActionService';
        let tmpActionSwitch = this.createStatlessSwitch(actionName, actionId, (value) => {
          this.setActionSwitchOn(value, actionKey);
        });
  ...
  setActionSwitchOn(state, actionName) {
    if (this.isMiotDeviceConnected()) {
      this.getDevice().fireAction(actionName);
  ...
  addPropValueListService(serviceName, prop, linkedProp) {
    let newCustService = new PropValueListService(serviceName, this, prop, linkedProp, this.api, this.logger);
    if (newCustService && newCustService.isServiceValid()) {
      this.logger.debug(`Adding custom service ${serviceName} for property ${prop.getName()}!`);
