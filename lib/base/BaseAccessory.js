...
class BaseAccessory {
  ...
  setLedBrightness(value) {
    if (this.isMiotDeviceConnected()) {
      ...
      this.ledBrightnessTimeout = setTimeout(() => this.getDevice().setLedValue(value), 500);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  updateDeviceStatus() {
    ...
    if (this.batteryService) this.batteryService.getCharacteristic(Characteristic.StatusLowBattery).updateValue(this.getBatteryLevelStatus());
