...
class BaseAccessory extends AbstractAccessory {
  ...
  prepareRelativeHumidityService() {
    if (this.getDevice().supportsRelativeHumidityReporting()) {
      ...
      this.relativeHumidityService.setCharacteristic(Characteristic.ConfiguredName, 'Humidity');
  ...
  prepareBatteryService() {
    if (this.getDevice().supportsBatteryLevelReporting() || this.getDevice().supportsChargingReporting()) {
      ...
      this.batteryService.setCharacteristic(Characteristic.ConfiguredName, 'Battery');
      this.batteryService
        .getCharacteristic(Characteristic.StatusLowBattery)
        .onGet(this.getStatusLowBatteryState.bind(this));
  ...
  prepareFilterMaintenanceService() {
    if (this.getDevice().supportsFilterLifeLevelReporting()) {
      ...
      this.filterMaintenanceService.setCharacteristic(Characteristic.ConfiguredName, 'Filter Maintenance');
  ...
  prepareAirQualityService(pm25Breakpoints) {
    if (this.getDevice().supportsPm25DensityReporting()) {
      ...
      if (this.getDevice().supportsTvocDensityReporting()) {
        this.airQualityService
          .getCharacteristic(Characteristic.VOCDensity)
          .onGet(this.getVOCDensity.bind(this))
          .setProps({
            minValue: this.getDevice().tvocDensityRange()[0],
            maxValue: this.getDevice().tvocDensityRange()[1],
            minStep: this.getDevice().tvocDensityRange()[2]
          });
      }
  ...
  prepareMethodButtonServices(methodButtonsUserConfig) {
    if (methodButtonsUserConfig && Array.isArray(methodButtonsUserConfig) && methodButtonsUserConfig.length > 0) {
      ...
      methodButtonsUserConfig.forEach((userMethod, i) => {
        ...
        let methodId = methodName + 'MethodService' + i; // generate action id
  ...
  updateAccessoryStatus() {
    ...
    if (this.airQualityService && this.getDevice().supportsTvocDensityReporting()) this.airQualityService.getCharacteristic(Characteristic.VOCDensity).updateValue(this.getVOCDensity());
