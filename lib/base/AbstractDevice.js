...
class AbstractDevice extends EventEmitter {
  ...
  setHomeKitNameResolver(resolver) {
    this.homeKitNameResolver = resolver;
  }

  resolveHomeKitName(defaultName, context) {
    if (!this.homeKitNameResolver) return defaultName;
    return this.homeKitNameResolver.resolve(defaultName, context);
  }

  formatHomeKitValueListName(propertyName, valueName, context) {
    if (!this.homeKitNameResolver) return `${propertyName} - ${valueName}`;
    return this.homeKitNameResolver.formatValueListName(propertyName, valueName, context);
  }
