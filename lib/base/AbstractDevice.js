...
class AbstractDevice extends EventEmitter {
  ...
  async initDeviceMatterAccessory(uuid, config, api, cachedDeviceInfo, restoredMatterAccessory, options = {}) {
    ...
    if (this.getType() !== DevTypes.ROBOT_CLEANER) {
      this.logger.error('Matter accessory is only implemented for robot cleaners.');
      return;
    }
    this.matterAccessoryWrapper = await new RobotCleanerMatterAccessory(this.getName(), this, uuid, config, api, this.logger, cachedDeviceInfo, restoredMatterAccessory, options).init();
