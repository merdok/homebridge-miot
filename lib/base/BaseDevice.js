...
class BaseDevice extends EventEmitter {
  ...
  async initDevice() {
    ...
    // set which properties to monitor
    const propsToMonitor = this._getPropsToMonitor();
    this.getMiotDevice().setPropertiesToMonitor(propsToMonitor);
    const filteredPropsToMonitor = this.getMiotDevice().getPropertiesToMonitor();
    if (propsToMonitor.length > 0) {
      this.logger.debug(`Initial properties to monitor: ${JSON.stringify(filteredPropsToMonitor, null, 2)}`);
    } else {
      this.logger.debug(`!-!-! Monitoring all properties !-!-!`);
    }
  ...
  _getPropsToMonitor() {
    const deviceSpecificPropsToMonitor = this.propertiesToMonitor();
    let propsToMonitor = deviceSpecificPropsToMonitor;
    if (deviceSpecificPropsToMonitor.length > 0) { //empty array = all properties, if not all specified then appen the common properties
      const commonPropsToMonitor = this.commonProperties();
      propsToMonitor = [...new Set([...deviceSpecificPropsToMonitor, ...commonPropsToMonitor])];
    }
