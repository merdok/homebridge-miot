...
class BaseDevice extends MiotDevice {
  ...
  powerOffTimerUnit() {
    return this.supportsPowerOffTimer() ? this.getPropertyUnit(Properties.POWER_OFF_TIME) : PropUnit.MINUTES;
  }
  ...
  supportsLedControlBrightness() {
    return this.supportsLedControlRange() && this.ledControlRange()[1] === 100 && this.getPropertyUnit(Properties.LED) === PropUnit.PERCENTAGE;
  }
  ...
  isFanSpeedRpm() {
    return this.getPropertyUnit(Properties.FAN_SPEED) === PropUnit.RPM;
  }
  ...
  // color
  supportsColor() {
    return this.hasProperty(Properties.COLOR);
  }
  ...
  useTimeUnit() {
    return this.supportsUseTimeReporting() ? this.getPropertyUnit(Properties.USE_TIME) : PropUnit.MINUTES;
  }
  ...
  supportsAirQualityReporting() {
    return this.hasProperty(Properties.AIR_QUALITY);
  }
  ...
  getColor() {
    return this.getPropertyValue(Properties.COLOR);
  }
  ...
  getAirQuality() {
    return this.getPropertyValue(Properties.AIR_QUALITY);
  }
  ...
  async setColor(colorRgb) {
    this.setPropertyValue(Properties.COLOR, colorRgb);
  }
  ...
  getFanSpeedPercentage() {
    if (this.supportsSteplesFanSpeed()) {
      if (this.isFanSpeedRpm()) {
        return this.convertPropValueToPercentage(Properties.FAN_SPEED);
      } else {
  ...
  async setFanSpeedPercentage(percentage) {
    if (this.supportsSteplesFanSpeed()) {
      let fanSpeedToSet = percentage;
      if (this.isFanSpeedRpm()) {
        fanSpeedToSet = this.convertPercentageToPropValue(percentage, Properties.FAN_SPEED)
      }
      this.setFanSpeed(fanSpeedToSet);
    } else if (this.supportsSteplessFanSpeedEmulation()) {
      let levelToSet = this.convertFanSpeedToFanLevel(percentage);
  ...
  setHue(hue) {
    let colorRgb = this.getColorRgb(hue, this.getSaturation(), this.getBrightness());
    return this.setColor(colorRgb);
  }

  setSaturation(saturation) {
    let colorRgb = this.getColorRgb(this.getHue(), saturation, this.getBrightness());
    return this.setColor(colorRgb);
  }
  ...
  getColorRgb(hue, saturation, brightness) {
    const color = convert.hsv.rgb([hue, saturation, brightness])
    const r = color[0];
    const g = color[1];
    const b = color[2];
