...
class BaseDevice extends EventEmitter {
  ...
  async initDevice(isReducedPropertyMonitoring = false) {
    ...
    const propsToMonitor = this._getInitialPropsToMonitor(isReducedPropertyMonitoring);
  ...
  _getInitialPropsToMonitor(isReducedPropertyMonitoring = false) {
    let propsToMonitor = [];
    ...
    // if reducded property monitoring flag is set then even if the device specified all properties to be monitored we skip that and only use the necessary ones
    if (!isReducedPropertyMonitoring && deviceSpecificPropsToMonitor !== null && deviceSpecificPropsToMonitor.length === 0) {
      return propsToMonitor;
    }

    if (isReducedPropertyMonitoring && deviceSpecificPropsToMonitor !== null && deviceSpecificPropsToMonitor.length === 0) {
      this.logger.debug(`#+#+ Using reduced property monitoring! +#+#`);
    }
