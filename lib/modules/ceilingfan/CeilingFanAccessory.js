...
class CeilingFanAccessory extends BaseAccessory {
  ...
  initAccessories(name, uuid) {
    return [new Accessory(name, uuid, this.api.hap.Categories.FAN)];
  }
  ...
  prepareModeControlServices() {
    ...
    if (this.getDevice().supportsLightModes()) {
      this.addPropWrapper('Light Mode', this.getDevice().lightModeProp(), this.getDevice().lightOnProp(), null, null, null);
    }
  }
