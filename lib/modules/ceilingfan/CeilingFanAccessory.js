...
class CeilingFanAccessory extends BaseAccessory {
  ...
  setupMainAccessoryService() {
    ...
    if (this.getDevice().supportsSteplessFanSpeed()) {
      this.fanService
  ...
  setupAdditionalAccessoryServices() {
    ...
    if (this.fanModeControl) this.prepareFanModeControlServices();
  ...
  prepareFanModeControlServices() {
    if (this.getDevice().supportsModes()) {
      ...
      this.getDevice().modes().forEach((mode, i) => {
        ...
        let tmpFanModeSwitch = this.createStatefulSwitch('Mode - ' + modeDesc, 'fanModeControlService' + modeVal, () => {
  ...
  prepareLightService() {
    if (this.getDevice().hasBuiltInLight()) {
      ...
      if (this.getDevice().supportsLightBrightness()) {
        this.lightService
      ...
      if (this.getDevice().supportsLightColorTemp()) {
        this.lightService
  ...
  getRotationSpeed() {
    if (this.isMiotDeviceConnected()) {
      return this.getDevice().getRotationSpeed();
    }
    return 0;
  }
  ...
  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      ...
      this.rotationSpeedTimeout = setTimeout(() => this.getDevice().setRotationSpeed(value), 500);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  updateDeviceStatus() {
    ...
    if (this.fanService && this.getDevice().supportsSteplessFanSpeed()) this.fanService.getCharacteristic(Characteristic.RotationSpeed).updateValue(this.getRotationSpeed());
    this.updateFanLevelSwitches();
    this.updateModeSwitches();

    if (this.lightService) this.lightService.getCharacteristic(Characteristic.On).updateValue(this.isLightOn());
    if (this.lightService && this.getDevice().supportsLightBrightness()) this.lightService.getCharacteristic(Characteristic.Brightness).updateValue(this.getLightBrightness());
    if (this.lightService && this.getDevice().supportsLightColorTemp()) this.lightService.getCharacteristic(Characteristic.ColorTemperature).updateValue(this.getLightColorTemp());
