...
class CeilingFanAccessory {
  ...
  setLightModeSwitchState(state, mode) {
    if (this.isMiotDeviceConnected()) {
      if (state) {
        ...
        if (this.miotCeilingFanDevice.isLightOn() === false) {
          this.miotCeilingFanDevice.setLightOn(true);
        }
  ...
  turnDeviceOnIfNecessary() {
    ...
    if (this.miotCeilingFanDevice.isPowerOn() === false) {
      this.miotCeilingFanDevice.setPowerOn(true);
    }
  }
