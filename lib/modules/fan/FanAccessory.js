  ...
  checkAngleButtonValue(angleValue) {
    if (this.getDevice().supportsHorizontalSwingAngle()) {
      ...
      if (this.getDevice().checkHorizontalSwingAngleWithinRange(angleValue) === false) {
        this.logger.warn(`Specified angle ${angleValue} is not within the supported range ${JSON.stringify(this.getDevice().horizontalSwingAngleRange())}. Not adding angle button!`);
        return false;
      }
      ...
      if (this.getDevice().checkHorizontalSwingLevelSupported(angleValue) === false) {
        this.logger.warn(`Specified angle ${angleValue} is not within the supported angle levels of your fan. Allowed values: ${JSON.stringify(this.getDevice().horizontalSwingLevels())}. Not adding angle button!`);
        return false;
      }
    }
