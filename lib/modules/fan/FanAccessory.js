...
class FanAccessory {
  ...
  prepareFanService() {
    ...
    if (this.miotFanDevice.supportsRotationSpeed()) {
      this.fanService
  ...
  getRotationSpeed() {
    if (this.isMiotDeviceConnected()) {
      return this.miotFanDevice.getFanSpeed();
    }
    return 0;
  }
  ...
  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      ...
      this.rotationSpeedTimeout = setTimeout(() => this.miotFanDevice.setFanSpeed(value), 500);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  updateDeviceStatus() {
    if (this.miotFanDevice) {
      ...
      if (this.fanService && this.miotFanDevice.supportsRotationSpeed()) this.fanService.getCharacteristic(Characteristic.RotationSpeed).updateValue(this.getRotationSpeed());
