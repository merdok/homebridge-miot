...
class FanAccessory {
  constructor(name, miotDevice, uuid, log, config, api, logger) {
    ...
    this.naturalModeControl = this.getPropValue(config['naturalModeControl'], true);
    this.sleepModeControl = this.getPropValue(config['sleepModeControl'], true);
  ...
  updateDeviceStatus() {
    if (this.miotFanDevice) {
      ...
      if (this.fanService) this.fanService.getCharacteristic(Characteristic.LockPhysicalControls).updateValue(this.getLockPhysicalControls());
      if (this.buzzerService) this.buzzerService.getCharacteristic(Characteristic.On).updateValue(this.getBuzzer());
      if (this.ledService) this.ledService.getCharacteristic(Characteristic.On).updateValue(this.getLed());
      if (this.ledBrightnessService) this.ledBrightnessService.getCharacteristic(Characteristic.On).updateValue(this.getLed());
      ...
      if (this.naturalModeControlService) this.naturalModeControlService.getCharacteristic(Characteristic.On).updateValue(this.getNaturalMode());
      if (this.sleepModeControlService) this.sleepModeControlService.getCharacteristic(Characteristic.On).updateValue(this.getSleepMode());
