...
class FanAccessory extends BaseAccessory {
  ...
  setupAdditionalAccessoryServices() {
    ...
    if (this.horizontalMoveControl) this.prepareHorizontalMoveControlService();
    if (this.verticalMoveControl) this.prepareVerticalMoveControlService();
  ...
  prepareBatteryService() {
    if (this.getDevice().hasBuiltInBattery() && (this.getDevice().supportsBatteryLevelReporting() || this.getDevice().supportsBatteryPowerReporting())) {
      this.batteryService = new Service.BatteryService(this.name + ' Battery', 'batteryService');
      this.batteryService
        .setCharacteristic(Characteristic.StatusLowBattery, Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL);

      if (this.getDevice().supportsBatteryPowerReporting()) {
        this.batteryService
  ...
  getBatteryLevelStatus() {
    if (this.isMiotDeviceConnected()) {
      return this.getDevice().getBatteryLevel() <= Constants.BATTERY_LOW_THRESHOLD ? Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW : Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL;
    }
  ...
  updateDeviceStatus() {
    ...
    if (this.batteryService && this.getDevice().supportsBatteryPowerReporting()) this.batteryService.getCharacteristic(Characteristic.ChargingState).updateValue(this.getBatteryChargingState());
    ...
    if (this.batteryService && this.getDevice().supportsBatteryLevelReporting()) this.batteryService.getCharacteristic(Characteristic.StatusLowBattery).updateValue(this.getBatteryLevelStatus());
