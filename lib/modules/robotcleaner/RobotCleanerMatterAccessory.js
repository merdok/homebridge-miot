...
class RobotCleanerMatterAccessory {
  ...
  _buildPowerSourceCluster(existingCluster = {}) {
    ...
    const cluster = this._mergeCluster(existingCluster, {
      status: 0,
      order: 0,
      description: 'Battery',
      batPercentRemaining: batteryState.batPercentRemaining,
      batChargeLevel: batteryState.batChargeLevel,
      batReplaceability: 1
    });

    if (this.device.hasKnownMatterBatteryLevel() || !this._hasUsableBatteryPercent(cluster.batPercentRemaining)) {
  ...
  _hasUsableBatteryPercent(value) {
    const percentRemaining = Number(value);
    return Number.isFinite(percentRemaining) && percentRemaining > 0;
  }
