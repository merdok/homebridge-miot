...
class RobotCleanerDevice extends BaseDevice {
  ...
  async fireActionStrict(actionObj, paramValues = []) {
    ...
    if (!this.isConnected()) {
      throw new Error(`Cannot execute action ${action.getName()} because the robot is not connected.`);
    }
    ...
    const result = await miotDevice.miotAction(actionDef);
  ...
  async executeMethodStrict(methodName, paramValues = []) {
    ...
    if (!this.isConnected()) {
      throw new Error(`Cannot execute method ${methodName} because the robot is not connected.`);
    }

    this.logger.deepDebug(`Executing strict robot cleaner method! Method: ${methodName} Params: ${JSON.stringify(paramValues)}`);
    const result = await this.getMiotDevice().miotSend(methodName, paramValues);
  ...
  async _tryDiscoverRoborockRooms() {
    if (!this._isRoborockModel() || !this.isConnected()) {
      return [];
    }
