...
class RobotCleanerDevice extends GenericDevice {
  ...
  statusSweepingValue() {
    return this._getValueForStatus('Sweeping');
  }

  statusIdleValue() {
    return this._getValueForStatus('Idle');
  }

  statusPausedValue() {
    return this._getValueForStatus('Paused');
  }

  statusErrorValue() {
    return this._getValueForStatus('Error');
  }

  statusGoChargingValue() {
    return this._getValueForStatus('Go Charging');
  }

  statusChargingValue() {
    return this._getValueForStatus('Charging');
  }

  statusMopppingValue() {
    return this._getValueForStatus('Mopping');
  }

  statusUpdatingValue() {
    return this._getValueForStatus('Updating');
  }

  statusSleepValue() {
    return this._getValueForStatus('Sleep');
  }

  statusSweepingAndMoppingValue() {
    return this._getValueForStatus('Sweeping and Mopping');
  }
  ...
  startChargeAction() {
    return this.getAction('battery:start-charge');
  }
  ...
  isStatusMopping() {
    return this.getStatus() === this.statusMopppingValue();
  }
  ...
  _getValueForStatus(statusDesc) {
    if (statusDesc && this.supportsStatusReporting()) {
      const statusValList = this.getPropertyValueList(this.statusProp());
      if (statusValList) {
        let foundObj = statusValList.find(item => this._areStatusDescEqual(item.description, statusDesc));
        if (foundObj) {
  ...
  _areStatusDescEqual(statusDesc1, statusDesc2) {
    if (statusDesc1 && statusDesc2) {
      const normalizedStausDesc1 = statusDesc1.replace(/\s+/g, '').toLowerCase();
      const normalizedStausDesc2 = statusDesc2.replace(/\s+/g, '').toLowerCase();
      if (normalizedStausDesc1 === normalizedStausDesc2) {
        return true;
      }
    }
    return false;
