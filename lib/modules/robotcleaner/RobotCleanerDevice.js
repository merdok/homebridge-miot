...
class RobotCleanerDevice extends BaseDevice {
  ...
  supportsMatterIdentify() {
    return !!(this.identifyAction() || this.audioPositionAction() || this.audioPlaySoundAction() || this.alarmProp() || this._isRoborockModel());
  }
  ...
  async resumeMatterCleaning(cleanMode = MATTER_RVC_CLEAN_MODES.VACUUM) {
  ...
  async identifyMatterAccessory() {
    ...
    if (this._isRoborockModel()) {
      try {
        ...
        lastError = err;
        this.logger.debug(`Robot cleaner identify legacy find_me fallback failed: ${err.message}`);
      }
    }
  ...
  getMatterBatteryState() {
    const batteryPercent = this.hasKnownMatterBatteryLevel() ? this.getBatteryLevel() : 100;
    return {
  ...
  async _tryDiscoverMapActionRooms() {
    ...
    const mapIds = await this._discoverMatterMapIds();
  ...
  _discoverRoomsFromCachedProperties() {
    return [
      ...
      return prop ? parseRoomsFromUnknownPayload(prop.getValue()) : [];
    });
  }
