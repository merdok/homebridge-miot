...
class RockroboVacuumV1 extends RobotCleanerDevice {
  ...
  async _readLegacyMiioProperties(params = []) {
    ...
    return params.map(param => {
      const value = this._getLegacyMiioPropertyValue(status, param);
  ...
  async _readLegacyMiioDeviceInfo() {
    ...
    return {
      model: this.getModel() || 'rockrobo.vacuum.v1',
      fw_ver: 'unknown'
    };
  }
  ...
  async _readLegacyMiioStatus() {
    ...
    const result = await miotDevice.miioProtocol.send(miotDevice.ip, 'get_status', []);
  ...
  _getLegacyMiioPropertyValue(status, param = {}) {
    const key = `${param.siid}.${param.piid}`;
    if (key === '2.1') {
    ...
    if (key === '3.2') {
      return this._isLegacyMiioCharging(status.state) ? 1 : 2;
    }
    return undefined;
  }

  _mapLegacyMiioState(state) {
    if ([5, 11, 17, 18].includes(state)) {
      return this.statusSweepingValue();
    }
    if ([10].includes(state)) {
      return this.statusPausedValue();
    }
    if ([6].includes(state)) {
      return this.statusGoChargingValue();
    }
    if ([7].includes(state)) {
      return 6; // remote control
    }
    if ([8].includes(state)) {
      return this.statusChargingValue();
    }
    if ([12].includes(state)) {
      return this.statusErrorValue();
    }
    return this.statusIdleValue();
  }

  _isLegacyMiioCharging(state) {
    return [8].includes(state);
  }
