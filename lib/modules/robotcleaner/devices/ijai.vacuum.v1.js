...
class IjaiVacuumV1 extends IjaiVacuumV2 {
  ...
  async discoverMatterRooms() {
    ...
    if (rooms.length > 0) {
      ...
      ].filter(room => room && room.id != null).map(room => [String(room.id), room])).values());
    }
  ...
  async _discoverCurrentMapRooms() {
    ...
    try {
      ...
      const roomPayload = this._getIjaiActionOutValue(result, 17);
  ...
  async _getCurrentIjaiMapId() {
    ...
    const mapListPayload = this._getIjaiActionOutValue(result, 4);
  ...
  _getIjaiActionOutValue(result, piid) {
