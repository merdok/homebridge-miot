...
class OvenDevice extends BaseDevice {
  ...
  // cook time
  supportsCookTimeReporting() {
    return this.hasProperty(Properties.COOK_TIME);
  }

  cookTimeRange() {
    let range = this.getPropertyValueRange(Properties.COOK_TIME);
    return (range.length > 2) ? range : [0, 100000, 1];
  }
  ...
  getCookTime() {
    return this.getPropertyValue(Properties.COOK_TIME);
  }
