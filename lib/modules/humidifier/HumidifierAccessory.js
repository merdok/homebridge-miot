...
class HumidifierAccessory {
  constructor(name, miotDevice, uuid, log, config, api, logger) {
    ...
    try {
      ...
      if (this.getAccessoryType() !== miotDevice.getType()) throw new Error(`Accessory type ${this.getAccessoryType()} cannot be used for device type ${this.getAccessoryType()}!`);
    } catch (error) {
  ...
  prepareHumidifierService() {
    ...
    if (this.miotHumidifierDevice.supportsRotationSpeed()) {
      this.humidifierService
  ...
  getRotationSpeed() {
    if (this.isMiotDeviceConnected()) {
      return this.miotHumidifierDevice.getFanSpeed();
    }
    return 0;
  }
  ...
  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      this.miotHumidifierDevice.setFanSpeed(value);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  setFanLevelSwitchState(state, level) {
    if (this.isMiotDeviceConnected()) {
      if (state) {
        this.miotHumidifierDevice.turnOnIfNecessary();
  ...
  updateDeviceStatus() {
    if (this.miotHumidifierDevice) {
      ...
      if (this.humidifierService && this.miotHumidifierDevice.supportsRotationSpeed()) this.humidifierService.getCharacteristic(Characteristic.RotationSpeed).updateValue(this.getRotationSpeed());
