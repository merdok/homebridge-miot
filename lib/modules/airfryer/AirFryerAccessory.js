...
class AirFryerAccessory extends BaseAccessory {
  ...
  initAccessories(name, uuid) {
    return [new Accessory(name, uuid, this.api.hap.Categories.AIR_HEATER)];
  }
  ...
  prepareTargetTimeService() {
    if (this.getDevice().supportsTargetTime()) {
      this.targetTimeService = new Service.Lightbulb(this.sanitizeName('Target Time'), 'targetTimeService');
      this.setServiceConfiguredName(this.targetTimeService, 'Target Time');
