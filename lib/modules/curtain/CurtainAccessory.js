...
class CurtainAccessory extends BaseAccessory {
  ...
  initConfigProperties(config) {
    ...
    this.motorReverseSwitch = this.getPropValue(config['motorReverseSwitch'], false);
  }
  ...
  initAccessory() {
    this.accessory = new Accessory(this.getName(), this.getUuid(), this.api.hap.Accessory.Categories.WINDOW_COVERING);
  }

  setupMainAccessoryService() {
    this.windowCoveringService = new Service.WindowCovering(this.name, 'windowCoveringService');
  ...
  setupAdditionalAccessoryServices() {
    ...
    this.prepareMotorReverseSwitchService();
  }
  ...
  prepareMotorControlServices() {
    if (this.motorControl && this.getDevice().supportsMotorControls()) {
      ...
      this.getDevice().motorControls().forEach((motorControl, i) => {
        ...
        let tmpMotorControlSwitch = this.createStatlessSwitchService('Motor Control - ' + motorControlDesc, 'motorControlService' + motorControlVal, (value) => {
  ...
  prepareMotorReverseSwitchService() {
    if (this.motorReverseSwitch) {
  ...
  getMotorReverseValue() {
    if (this.isMiotDeviceConnected()) {
      return this.getDevice().isMotorReverse();
    }
    return false;
  }
  ...
  setMotorReverseValue(value) {
    if (this.isMiotDeviceConnected()) {
      this.getDevice().setMotorReverse(value);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
