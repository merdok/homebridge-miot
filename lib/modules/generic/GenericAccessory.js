...
class GenericAccessory {
  ...
  initAccessory() {
    ...
    this.accessory = new Accessory(this.name, this.uuid, this.api.hap.Accessory.Categories.SWITCH);
  ...
  setPowerState(state) {
    if (this.isMiotDeviceConnected()) {
      this.miotGenericDevice.setPowerOn(state);
    } else {
  ...
  turnDeviceOnIfNecessary() {
    ...
    if (this.miotGenericDevice.isPowerOn() === false) {
      this.miotGenericDevice.setPowerOn(true);
    }
  }
