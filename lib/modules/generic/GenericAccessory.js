...
class GenericAccessory extends BaseAccessory {
  ...
  addColorTemperatureCharacteristic(service) {
    if (this.getDevice().supportsColorTemperature() && service) {
  ...
  setBrightness(brightness) {
    if (this.isMiotDeviceConnected()) {
      ...
      this.brightnessTimeout = setTimeout(() => this.getDevice().setBrightness(brightness), Constants.SLIDER_DEBOUNCE);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  getColorTemperature() {
    if (this.isMiotDeviceConnected()) {
      return this.getDevice().getColorTempMired();
    }
  ...
  setColorTemperature(colorTemp) {
    if (this.isMiotDeviceConnected()) {
      this.getDevice().setColorTempMired(colorTemp);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  updateAccessoryStatus() {
    ...
    if (this.airQualityService && this.getDevice().supportsPm10DensityReporting()) this.airQualityService.getCharacteristic(Characteristic.PM10Density).updateValue(this.getPM10Density());
