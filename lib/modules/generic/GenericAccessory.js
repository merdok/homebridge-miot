...
class GenericAccessory {
  ...
  getPowerState() {
    if (this.miotGenericDevice && this.miotGenericDevice.isConnected()) {
  ...
  updateDeviceStatus() {
    if (this.miotGenericDevice) {
      if (this.powerSwitchService) this.powerSwitchService.getCharacteristic(Characteristic.On).updateValue(this.miotFanDevice.isPowerOn());
    }
  }
