...
class HeaterAccessory extends BaseAccessory {
  ...
  prepareHeatLevelControlServices() {
    if (this.getDevice().supportsHeatLevels()) {
      this.addPropValueListWrapper('Heat', Properties.HEAT_LEVEL, Properties.POWER);
    }
  }

  prepareFanService() {
    if (this.getDevice().supportsFan()) {
      this.fanService = this.createStatefulSwitch('Fan', 'fanService', this.isFanOn, this.setFanOn);
  ...
  updateDeviceStatus() {
    ...
    if (this.fanService) this.fanService.getCharacteristic(Characteristic.On).updateValue(this.isFanOn());
