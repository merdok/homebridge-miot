...
class HeaterAccessory extends BaseAccessory {
  ...
  setupMainAccessoryService() {
    ...
    if (this.getDevice().supportsTargetTemperature()) {
      ...
      if (this.getDevice().supportsCoolMode()) {
        this.heaterService
          .getCharacteristic(Characteristic.TargetHeaterCoolerState)
          .setProps({
      ...
      if (this.getDevice().supportsTargetTemperatureRange()) {
        this.heaterService
          .getCharacteristic(Characteristic.HeatingThresholdTemperature)
          .setProps({
            minValue: this.getDevice().targetTemperatureRange()[0] || 10,
            maxValue: this.getDevice().targetTemperatureRange()[1] || 35,
            minStep: this.getDevice().targetTemperatureRange()[2] || 1
          });
      }

      this.heaterService
        .getCharacteristic(Characteristic.TemperatureDisplayUnits)
        //  .onGet(this.getTemperatureDisplayUnitsState.bind(this))
        //  .onSet(this.setTemperatureDisplayUnitsState.bind(this));
  ...
  prepareHeatLevelControlServices() {
    if (this.getDevice().supportsHeatLevels()) {
      ...
      this.getDevice().heatLevels().forEach((heatLevel, i) => {
        let heatLevelVal = heatLevel.value;
        let heatLevelDesc = heatLevel.description;
        let tmpHeatLevelSwitch = this.createStatefulSwitch('Heat - ' + heatLevelDesc, 'heatLevelControlService' + heatLevelVal, () => {
          return this.isHeatLevelSwitchOn(heatLevelVal);
        }, (value) => {
          this.setHeatLevelSwitchOn(value, heatLevelVal);
        });
        this.addAccessoryService(tmpHeatLevelSwitch);
  ...
  getCurrentHeaterCoolerState() {
    if (this.isMiotDeviceConnected()) {
      return this.getDevice().isPowerOn() ? Characteristic.CurrentHeaterCoolerState.HEATING : Characteristic.CurrentHeaterCoolerState.IDLE;
    }
  ...
  getTargetHeaterCoolerState() {
    if (this.isMiotDeviceConnected() && this.getDevice().isPowerOn()) {
      if (this.getDevice().supportsModes()) {
        if (this.getDevice().isAutoModeEnabled()) {
          return Characteristic.TargetHeaterCoolerState.AUTO;
        } else if (this.getDevice().isHeatModeEnabled()) {
          return Characteristic.TargetHeaterCoolerState.HEAT;
        } else if (this.getDevice().isCoolModeEnabled()) {
  ...
  setTargetHeaterCoolerState(state) {
    if (this.isMiotDeviceConnected()) {
      if (this.getDevice().supportsModes()) {
        ...
        this.getDevice().setTargetTemperature(this.getDevice().getTargetTemperature());
      }
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  ...
  getHeatingThresholdTemperature() {
    ...
    if (this.getDevice().supportsTargetTemperatureRange()) {
      ...
      return this.getDevice().targetTemperatureRange()[0];
    }
    return 0;
  }
  ...
  isHeatLevelSwitchOn(level) {
    if (this.isMiotDeviceConnected() && this.getDevice().isPowerOn()) {
      return this.getDevice().getHeatLevel() === level;
    }
    return false;
  }

  setHeatLevelSwitchOn(state, level) {
    if (this.isMiotDeviceConnected()) {
      if (state) {
  ...
  updateHeatLevelSwitches() {
    if (this.heatLevelControlServices) {
      let currentLevel = this.getDevice().getHeatLevel();
      this.heatLevelControlServices.forEach((tmpHeatLevelSwitch, i) => {
        let heatLevel = this.getDevice().heatLevels()[i];
        let heatLevelVal = heatLevel.value;
        let isSwitchOn = (currentLevel === heatLevelVal) && this.getDevice().isPowerOn();
