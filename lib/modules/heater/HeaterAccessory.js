...
class HeaterAccessory {
  ...
  getTargetHeaterCoolerState() {
    if (this.isMiotDeviceConnected() && this.miotHeaterDevice.isPowerOn()) {
      return this.miotHeaterDevice.isPowerOn() ? Characteristic.TargetHeaterCoolerState.HEAT : Characteristic.TargetHeaterCoolerState.AUTO;
    }
  ...
  setTargetHeaterCoolerState(state) {
    if (this.isMiotDeviceConnected()) {
      if (state === Characteristic.TargetHeaterCoolerState.HEAT) {
        this.miotHeaterDevice.setHeatEnabled();
      } else {
        this.miotHeaterDevice.setAutoEnabled();
      }
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
