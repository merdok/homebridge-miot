...
class HeaterAccessory {
  ...
  setupAccessoryServices() {
    ...
    this.prepareSwingControlService();
  ...
  prepareSwingControlService() {
    if (this.swingControl && this.miotHeaterDevice.supportsSwingModes()) {
      this.swingControlService = new Service.Switch(this.name + ' Swing', 'swingControlService');
      this.swingControlService
        .getCharacteristic(Characteristic.On)
        .onGet(this.getSwingSwitch.bind(this))
        .onSet(this.setSwingSwitch.bind(this));
      this.accessory.addService(this.swingControlService);
    }
  }
  ...
  prepareHeatLevelControlServices() {
    if (this.heatLevelControl && this.miotHeaterDevice.supportsHeatLevels()) {
      ...
      this.miotHeaterDevice.heatLevels().forEach((heatLevel, i) => {
        ...
        let tmpHeatLevelSwitch = new Service.Switch(this.name + ' Heat ' + heatLevelName, 'heatLevelControlService' + heatLevelValue);
        tmpHeatLevelSwitch
          .getCharacteristic(Characteristic.On)
          .onGet(() => {
  ...
  getSwingSwitch() {
    if (this.isMiotDeviceConnected()) {
      return this.miotHeaterDevice.isFanSwingModeEnabled();
    }
    return false;
  }

  setSwingSwitch(state) {
    if (this.isMiotDeviceConnected()) {
      if (state) {
        this.miotHeaterDevice.enableFanSwingMode();
      } else {
  ...
  updateDeviceStatus() {
    if (this.miotHeaterDevice) {
      ...
      if (this.swingControlService) this.swingControlService.getCharacteristic(Characteristic.On).updateValue(this.getSwingSwitch());
    ...
    if (this.heatLevelControlServices) {
      ...
      this.heatLevelControlServices.forEach((tmpHeatLevelSwitch, i) => {
        ...
        if (currentLevel === heatLevelValue && this.miotHeaterDevice.isPowerOn()) {
          tmpHeatLevelSwitch.getCharacteristic(Characteristic.On).updateValue(true);
        } else {
          tmpHeatLevelSwitch.getCharacteristic(Characteristic.On).updateValue(false);
        }
      });
    }
  }
