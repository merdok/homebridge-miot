...
class ZhimiAirfreshUa1 extends FreshAirSystemDevice {
  ...
  initDeviceProperties() {
    ...
    // NOTE: siid 8 piid 5 (inlet temperature) is intentionally not exposed. This device's firmware
    // returns corrupted JSON for this property when the reading drops below zero (e.g. `"value":-0.)`),
    // which fails to parse and times out the whole poll, eventually stopping polling for the device.
    // See issue #734. Skipping the property keeps the rest of the device working in cold conditions.
  }
  ...
  // temperatureProp intentionally not overridden - the device's inlet temperature (siid 8 piid 5)
  // is not exposed because its firmware sends corrupted JSON for sub-zero values (issue #734).
