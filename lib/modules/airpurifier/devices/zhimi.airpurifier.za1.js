...
class ZhimiAirPurifierZa1 extends AirPurifierDevice {
  ...
  initDeviceProperties() {
    this.addProperty(AirPurifierProperties.POWER, 2, 1, PropFormat.BOOL, ['read', 'write', 'notify'], Constants.PROP_UNIT_NONE, null);
    ...
    this.addProperty(AirPurifierProperties.FAVORITE_SPEED, 10, 10, PropFormat.INT32, ['read', 'write'], Constants.PROP_UNIT_NONE, [0, 14, 1]);
    ...
    this.addProperty(AirPurifierProperties.COUNTRY_CODE, 15, 12, PropFormat.UINT8, ['read', 'write', 'notify'], Constants.PROP_UNIT_NONE, null);

    this.addProperty(AirPurifierProperties.AQI_VALUE, 3, 1, PropFormat.UINT16, ['read', 'notify'], Constants.PROP_UNIT_NONE, [0, 500, 1]);
    this.addProperty(AirPurifierProperties.PM25_DENSITY, 3, 6, PropFormat.UINT16, ['read', 'notify'], Constants.PROP_UNIT_NONE, [0, 600, 1]);
    ...
    this.addProperty(AirPurifierProperties.TEMPERATURE, 3, 8, PropFormat.FLOAT, ['read', 'notify'], Constants.PROP_UNIT_CELSIUS, [-30, 100, 0.1]);
    ...
    this.addProperty(AirPurifierProperties.FILTER_USED_TIME, 4, 5, PropFormat.UINT16, ['read', 'notify'], Constants.PROP_UNIT_HOURS, [0, 15000, 1]);
    this.addProperty(AirPurifierProperties.FAN_SPEED_RPM, 10, 11, PropFormat.UINT16, ['read'], Constants.PROP_UNIT_RPM, [0, 3000, 1]);
    this.addProperty(AirPurifierProperties.USE_TIME, 12, 1, PropFormat.UINT32, ['read'], Constants.PROP_UNIT_SECONDS, [0, 2147483600, 1]);
    this.addProperty(AirPurifierProperties.AQI_STATE, 13, 4, PropFormat.STRING, ['read'], Constants.PROP_UNIT_NONE, null); // marked as aqi zone, so is this the state in text form?
  }
