...
class ZhimiAirPurifierZa1 extends AirPurifierDevice {
  ...
  getDeviceMiotSpec() {
    return "http://miot-spec.org/miot-spec-v2/instance?type=urn:miot-spec-v2:device:air-purifier:0000A007:zhimi-za1:1";
  }
  ...
  initDeviceProperties() {
    ...
    this.addProperty(Properties.COUNTRY_CODE, 15, 12, PropFormat.UINT8, PropAccess.READ_WRITE_NOTIFY, PropUnit.NONE);

    // READ ONLY
    this.addProperty(Properties.AIR_QUALITY, 3, 1, PropFormat.UINT16, PropAccess.READ_NOTIFY, PropUnit.NONE, [0, 500, 1]);
    this.addProperty(Properties.PM25_DENSITY, 3, 6, PropFormat.UINT16, PropAccess.READ_NOTIFY, PropUnit.NONE, [0, 600, 1]);
    this.addProperty(Properties.RELATIVE_HUMIDITY, 3, 7, PropFormat.UINT8, PropAccess.READ_NOTIFY, PropUnit.PERCENTAGE, [0, 100, 1]);
    this.addProperty(Properties.TEMPERATURE, 3, 8, PropFormat.FLOAT, PropAccess.READ_NOTIFY, PropUnit.CELSIUS, [-30, 100, 0.1]);
    this.addProperty(Properties.FILTER_LIFE_LEVEL, 4, 3, PropFormat.UINT8, PropAccess.READ_NOTIFY, PropUnit.PERCENTAGE, [0, 100, 1]);
    this.addProperty(Properties.FILTER_USED_TIME, 4, 5, PropFormat.UINT16, PropAccess.READ_NOTIFY, PropUnit.HOURS, [0, 15000, 1]);
    this.addProperty(Properties.FAN_SPEED_RPM, 10, 11, PropFormat.UINT16, PropAccess.READ, PropUnit.RPM, [0, 3000, 1]);
    this.addProperty(Properties.USE_TIME, 12, 1, PropFormat.UINT32, PropAccess.READ, PropUnit.SECONDS, [0, 2147483600, 1]);
    this.addProperty(Properties.AQI_STATE, 13, 4, PropFormat.STRING, PropAccess.READ, PropUnit.NONE); // marked as aqi zone, so is this the state in text form?
  }
