...
class ZhimiAirPurifierZa1 extends AirPurifierDevice {
  ...
  initDeviceProperties() {
    ...
    this.addProperty(AirPurifierProperties.FAVORITE_SPEED, 10, 10, 'int32', ['read', 'write'], Constants.PROP_UNIT_LEVEL, [0, 14, 1]);
    ...
    this.addProperty(AirPurifierProperties.COUNTRY_CODE, 15, 12, 'uint8', ['read', 'write', 'notify'], Constants.PROP_UNIT_NONE, null);

    this.addProperty(AirPurifierProperties.AQI_VALUE, 3, 1, 'uint16', ['read', 'notify'], Constants.PROP_UNIT_NONE, [0, 500, 1]);
    this.addProperty(AirPurifierProperties.PM25_DENSITY, 3, 6, 'uint16', ['read', 'notify'], Constants.PROP_UNIT_NONE, [0, 600, 1]);
    this.addProperty(AirPurifierProperties.RELATIVE_HUMIDITY, 3, 7, 'uint8', ['read', 'notify'], Constants.PROP_UNIT_PERCENTAGE, [0, 100, 1]);
    this.addProperty(AirPurifierProperties.TEMPERATURE, 3, 8, 'float', ['read', 'notify'], Constants.PROP_UNIT_CELSIUS, [-30, 100, 0.1]);
    this.addProperty(AirPurifierProperties.FILTER_LIFE_LEVEL, 4, 3, 'uint8', ['read', 'notify'], Constants.PROP_UNIT_PERCENTAGE, [0, 100, 1]);
    this.addProperty(AirPurifierProperties.FILTER_USED_TIME, 4, 5, 'uint16', ['read', 'notify'], Constants.PROP_UNIT_HOURS, [0, 15000, 1]);
    this.addProperty(AirPurifierProperties.FAN_SPEED_RPM, 10, 11, 'uint16', ['read'], Constants.PROP_UNIT_RPM, [0, 3000, 1]);
    this.addProperty(AirPurifierProperties.USE_TIME, 12, 1, 'uint32', ['read'], Constants.PROP_UNIT_SECONDS, [0, 2147483600, 1]);
    this.addProperty(AirPurifierProperties.AQI_STATE, 13, 4, 'string', ['read'], Constants.PROP_UNIT_NONE, null); // marked as aqi zone, so is this the state in text form?
  }
