...
class AirPurifierAccessory extends BaseAccessory {
  ...
  setupMainAccessoryService() {
    ...
    if (this.getDevice().supportsFavoriteSpeedControl()) {
      this.airPurifierService
        ...
        .setProps({
          minValue: this.getDevice().favoriteSpeedMinVal(),
          maxValue: this.getDevice().favoriteSpeedMaxVal(),
          minStep: this.getDevice().favoriteSpeedStepVal()
        });
    }
  ...
  getRotationSpeed() {
    if (this.isMiotDeviceConnected() && this.getDevice().isFavoriteModeEnabled()) {
      return this.getDevice().getFavoriteSpeedValue();
    }
    return this.getDevice().favoriteSpeedMinVal();
  }
  ...
  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      ...
      this.getDevice().setFavoriteSpeedValue(value);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  preparePm25Breakpoints() {
    ...
    if (this.pm25Breakpoints) {
      if (!Array.isArray(this.pm25Breakpoints) || this.pm25Breakpoints.length != 4 ||
