...
class AirPurifierAccessory {
  ...
  prepareAccessoryService() {
    ...
    if (this.miotDevice.supportsChildLock()) {
      this.airPurifierService
  ...
  getCurrentAirPurifierState() {
    if (this.isMiotDeviceConnected()) {
      return this.miotDevice.isPowerOn() ? Characteristic.CurrentAirPurifierState.PURIFYING_AIR : Characteristic.CurrentAirPurifierState.IDLE;
    }
  ...
  getRotationSpeed() {
    if (this.isMiotDeviceConnected()) {
      return this.miotDevice.getRotationSpeed();
    }
    return 0;
  }
  ...
  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      this.miotDevice.setRotationSpeed(value);
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  updateDeviceStatus() {
    if (this.miotDevice) {
      ...
      if (this.airPurifierService && this.miotDevice.supportsChildLock()) this.airPurifierService.getCharacteristic(Characteristic.LockPhysicalControls).updateValue(this.getLockPhysicalControls());
