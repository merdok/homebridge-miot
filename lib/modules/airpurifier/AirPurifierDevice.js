...
class AirPurifierDevice extends MiotDevice {
  ...
  initialPropertyFetchDone() {
    ...
    // log the total use time when supported
  ...
  supportsLedBrightnesssRange() {
    return this.lightBrightnessRange().length > 0 ? false : true;
  }
  ...
  getFavoriteSpeed() {
    return this.getPropertyValue(AirPurifierProperties.FAVORITE_SPEED);
  }
  ...
  isLedEnabled() {
    if(this.supportsLedBrightnesssRange()){
      ...
      return this.getPropertyValue(Properties.LIGHT) !== levels[numOfLevels-1]; // last one is usually off
    }
  ...
  async setFavoriteSpeed(speed) {
    this.setPropertyValue(AirPurifierProperties.FAVORITE_SPEED, mode);
  }
