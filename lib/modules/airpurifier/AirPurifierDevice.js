...
class AirPurifierDevice extends MiotDevice {
  ...
  supportsLedBrightnesssRange() {
    return this.ledBrightnessRange().length > 0 ? true : false;
  }
  ...
  isAutoModeEnabled() {
    if (this.supportsAutoMode()) {
      return this.getMode() === this.capabilities[AirPurifierCapabilities.AUTO_MODE_VALUE];
    }
    return false;
  }
  ...
  isSleepModeEnabled() {
    if (this.supportsSleepMode()) {
      return this.getMode() === this.capabilities[AirPurifierCapabilities.SLEEP_MODE_VALUE];
    }
    return false;
  }
  ...
  isFavoriteModeEnabled() {
    if (this.supportsFavoriteMode()) {
      return this.getMode() === this.capabilities[AirPurifierCapabilities.FAVORITE_MODE_VALUE];
    }
    return false;
  }
  ...
  isLedEnabled() {
    if (this.supportsLedBrightnesssRange()) {
      let minLevel = this.ledBrightnessRange()[0];
      let maxLevel = this.ledBrightnessRange()[1];
      return this.getPropertyValue(AirPurifierProperties.LIGHT) !== maxLevel; // last one is usually off
    }
  ...
  async enableAutoMode() {
    if (this.supportsAutoMode()) {
      this.setMode(this.capabilities[AirPurifierCapabilities.AUTO_MODE_VALUE]);
    }
  }

  async enableSleepMode() {
    if (this.supportsSleepMode()) {
      this.setMode(this.capabilities[AirPurifierCapabilities.SLEEP_MODE_VALUE]);
    }
  }

  async enableFavoriteMode() {
    if (this.supportsFavoriteMode()) {
      this.setMode(this.capabilities[AirPurifierCapabilities.FAVORITE_MODE_VALUE]);
    }
  }
