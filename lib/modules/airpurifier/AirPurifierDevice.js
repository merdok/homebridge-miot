...
class AirPurifierDevice extends MiotDevice {
  ...
  // led
  ledControlRange() {
    return this.capabilities[AirPurifierCapabilities.LED_CONTROL_RANGE] || [];
  }
  ...
  isLedEnabled() {
    if (this.supportsLedControlRange()) {
      ...
      return this.getPropertyValue(AirPurifierProperties.LED) !== maxLevel; // last one is usually off
    }
    return this.getPropertyValue(AirPurifierProperties.LED);
  }
  ...
  async setLedEnabled(enabled) {
    if (this.supportsLedControlRange()) {
      ...
      this.setPropertyValue(AirPurifierProperties.LED, level);
      return;
    }
    this.setPropertyValue(AirPurifierProperties.LED, enabled);
  }
