...
class BathHeaterAccessory extends BaseAccessory {
  ...
  prepareLightModeControlServices() {
    if (this.getDevice().supportsLightModes()) {
      ...
      this.lightModeServices = new Array();
      this.getDevice().lightModes().forEach((lightMode, i) => {
        let lightModeVal = lightMode.value;
        let lightModeDesc = lightMode.description;
        let tmpLightModeSwitch = this.createStatlessSwitch('Light Mode - ' + lightModeDesc, 'lightModeService' + lightModeVal, (value) => {
          this.setLightModeSwitchOn(value, lightModeVal);
        });
        this.addAccessoryService(tmpLightModeSwitch);
        this.lightModeServices.push(tmpLightModeSwitch);
      });
    }
  }
  ...
  setLightModeSwitchOn(value, lightModeValue) {
    if (this.isMiotDeviceConnected()) {
      this.getDevice().setLightMode(lightModeValue);
      this.resetLightModeSwitches();
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
  ...
  resetLightModeSwitches() {
    if (this.lightModeServices) {
      setTimeout(() => {
        this.lightModeServices.forEach((tmpLightModeSwitch, i) => {
          tmpLightModeSwitch.getCharacteristic(Characteristic.On).updateValue(false);
        });
