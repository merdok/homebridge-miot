...
class SwitchAccessory extends GenericAccessory {
  ...
  setupMainAccessoryService() {
    if (this.getDevice().hasMultiplySwitchServices()) {
      this.getDevice().getAllSwitchServices().forEach((service, i) => {
        ...
        if (serviceOnProp) {
          this.addPropWrapper(serviceDesc, serviceOnProp, null, null, null);
        }
      });
    } else {
  ...
  prepareModeControlServices() {
    if (this.getDevice().hasMultiplySwitchServices()) {
      this.getDevice().getAllSwitchServices().forEach((service, i) => {
        ...
        if (serviceModeProp) {
          this.addPropWrapper(`${serviceDesc} Mode`, serviceModeProp, null, null, null);
        }
      });
    } else {
