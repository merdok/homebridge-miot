...
class DeviceFactory {
  ...
  static createDevice(deviceModel, deviceId, name, logger) {
    ...
    if (deviceModel) {
      ...
      // air conditioner
      if (!deviceFactoryClass) {
        deviceFactoryClass = AirConditionerFactory.getDeviceClassForModel(deviceModel);
      }

      // airer
      if (!deviceFactoryClass) {
        deviceFactoryClass = AirerFactory.getDeviceClassForModel(deviceModel);
      }

      // oven
      if (!deviceFactoryClass) {
        deviceFactoryClass = OvenFactory.getDeviceClassForModel(deviceModel);
      }
