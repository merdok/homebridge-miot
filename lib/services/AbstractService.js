...
class AbstractService {
  constructor(serviceId, serviceName, miotService, device, accessory, api, logger) {
    ...
    if (new.target === AbstractService) {
      throw new Error('Cannot instantiate AbstractService directly!')
    }
  ...
  initService() {
    this.logger.deepDebug(`<-S-> Initializing ${this.getServiceName() || this.getServiceId()} custom service of type ${this.getServiceType()} for service ${this.getMiotService().getRawType()}`);
    ...
    try {
      ...
      this.isValid = false;
      this.logger.warn(`Failed to create custom service with name ${this.getServiceName() || this.getServiceId()}! Error during service creation! Skipping...`);
  ...
  getServiceName() {
    return this.localizeName(this.serviceName);
  }
  ...
  localizeName(name) {
    ...
    return this.getDevice().resolveHomeKitName(name, {
      kind: 'service',
      siid: this.getServiceSiid(),
    });
  }
