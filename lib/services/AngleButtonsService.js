...
const CustomServiceBase = require('./CustomServiceBase.js');
...
class AngleButtonsService extends CustomServiceBase {
...
console.log(this.getConfiguration());
    ...
    console.log(this.swingAnglesList);
  ...
  updateSwingAngleSwitches() {
    if (this.swingAngleControlServices && this.swingAnglesList && this.swingAnglesList.length > 0) {
      let currentAngle = this.getProp().getValue();
      this.swingAngleControlServices.forEach((tmpSwingAngleSwitch, i) => {
        let swingAngle = this.swingAnglesList[i];
        let swingAngleVal = swingAngle.value;
        let isSwitchOn = (currentAngle === swingAngleVal) && this.getDevice().isPowerOn() && this.checkLinkedPropStatus();
...
module.exports = AngleButtonsService;
