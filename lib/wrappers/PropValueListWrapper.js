...
class PropValueListWrapper extends AbstractPropertyWrapper {
  ...
  createStatefulSwitch(switchName, switchId, itemVal) {
    let tmpStatefulSwitch = new Service.Switch(this.sanitizeLocalizedName(switchName), switchId);
    this.setServiceConfiguredName(tmpStatefulSwitch, switchName, true);
    tmpStatefulSwitch
  ...
  createStatlessSwitch(switchName, switchId, itemVal) {
    let tmpStatlessSwitch = new Service.Switch(this.sanitizeLocalizedName(switchName), switchId);
    this.setServiceConfiguredName(tmpStatlessSwitch, switchName, true);
    tmpStatlessSwitch
      .getCharacteristic(Characteristic.On)
      .onGet(() => {
      ...
      .onSet((value) => {
        ...
        setTimeout(() => {
          if (tmpStatlessSwitch) tmpStatlessSwitch.getCharacteristic(Characteristic.On).updateValue(false);
        }, Constants.BUTTON_RESET_TIMEOUT);
      });
