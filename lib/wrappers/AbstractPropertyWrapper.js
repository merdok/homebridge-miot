...
const HomeKitUtils = require('../utils/HomeKitUtils.js');
...
class AbstractPropertyWrapper {
  ...
  setNameIsExplicit(nameIsExplicit) {
    this.nameIsExplicit = !!nameIsExplicit;
  }
  ...
  setServiceConfiguredName(service, name, isLocalized = false) {
    if (service) {
      service.addOptionalCharacteristic(Characteristic.ConfiguredName);
      service.setCharacteristic(Characteristic.ConfiguredName, isLocalized ? this.sanitizeLocalizedName(name) : this.sanitizeName(name));
    }
  }

  createStatefulSwitch(name, id, getterFn, setterFn, isLocalized = false) {
    let newStatefulSwitch = new Service.Switch(isLocalized ? this.sanitizeLocalizedName(name) : this.sanitizeName(name), id);
    this.setServiceConfiguredName(newStatefulSwitch, name, isLocalized);
    newStatefulSwitch
  ...
  createStatlessSwitch(name, id, setterFn, isLocalized = false) {
    let newStatelessSwitch = new Service.Switch(isLocalized ? this.sanitizeLocalizedName(name) : this.sanitizeName(name), id);
    this.setServiceConfiguredName(newStatelessSwitch, name, isLocalized);
    newStatelessSwitch
      .getCharacteristic(Characteristic.On)
  ...
  sanitizeName(name) {
    return HomeKitUtils.sanitizeHomeKitName(this.localizeName(name));
  }

  sanitizeLocalizedName(name) {
    return HomeKitUtils.sanitizeHomeKitName(name);
  }

  getUniqueLocalizedName(name, usedNames, suffixStart) {
    let uniqueName = name;
    let suffix = suffixStart;
    while (usedNames.has(this.sanitizeLocalizedName(uniqueName))) {
      uniqueName = `${name} ${suffix}`;
      suffix += 1;
    }
    usedNames.add(this.sanitizeLocalizedName(uniqueName));
    return uniqueName;
  }

  localizeName(name) {
    if (!this.getDevice() || !this.getDevice().resolveHomeKitName) return name;
    return this.getDevice().resolveHomeKitName(name, this.getLocalizationContext('property'));
  }

  localizeValueName(name, valueIndex) {
    if (!this.getDevice() || !this.getDevice().resolveHomeKitName) return name;
    return this.getDevice().resolveHomeKitName(name, this.getLocalizationContext('value', valueIndex));
  }

  localizeValueListName(propertyName, valueName, valueIndex) {
    const localizedPropertyName = this.localizeName(propertyName);
    const localizedValueName = this.localizeValueName(valueName, valueIndex);
    if (!this.getDevice() || !this.getDevice().formatHomeKitValueListName) {
      return `${localizedPropertyName} - ${localizedValueName}`;
    }
    return this.getDevice().formatHomeKitValueListName(
      localizedPropertyName,
      localizedValueName,
      {
        ...this.getLocalizationContext('value', valueIndex),
  ...
  getLocalizationContext(kind, valueIndex) {
    const prop = this.getProp();
    const device = this.getDevice();
    const service = device && device.getServiceById ? device.getServiceById(prop.getServiceId()) : null;
    return {
      explicit: this.nameIsExplicit,
      kind,
      nameSource: kind === 'property' ? 'wrapper' : 'miot',
      siid: prop.getServiceId(),
      piid: prop.getId(),
      valueIndex,
      sourceDescription: prop.getDescription ? prop.getDescription() : null,
      propertyType: prop.getType ? prop.getType() : null,
      serviceType: service && service.getRawType ? service.getRawType() : null,
    };
  }
