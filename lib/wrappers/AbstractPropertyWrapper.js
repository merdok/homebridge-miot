...
class AbstractPropertyWrapper {
  ...
  initWrapper() {
    this.logger.deepDebug(`<-W-> Initializing ${this.wrapperName} wrapper of type ${this.getWrapperType()} for property ${this.prop.getName()}`);

    // prepare the custom service
    this.isValid = this.prepareWrapper();

    if (this.isWrapperValid()) {
      this.getDevice().addPropertyToMonitor(this.prop);
    }
  }
