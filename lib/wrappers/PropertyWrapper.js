...
class PropertyWrapper extends AbstractPropertyWrapper {
  ...
  handleBoolean() {
    ...
    this.propBooleanService = new Service.Switch(this.sanitizeName(serviceName), serviceId);
    this.setServiceConfiguredName(this.propBooleanService, serviceName);
  ...
  handleValueRange() {
    ...
    if (isFan) {
      // fan
      this.propFanService = new Service.Fanv2(this.sanitizeName(serviceName), serviceId);
      this.setServiceConfiguredName(this.propFanService, serviceName);
      this.propFanService
      ...
      // light bulb
      this.propBrightnessService = new Service.Lightbulb(this.sanitizeName(serviceName), serviceId);
      this.setServiceConfiguredName(this.propBrightnessService, serviceName);
  ...
  handleValueList() {
    ...
    const usedSwitchNames = new Set();
    this.valueList().forEach((item, i) => {
      let itemVal = item.value;
      let itemDesc = typeof item.description === 'string' && item.description.trim() !== ''
        ? item.description
        : String(itemVal);
      let propName = this.getProp().getName();
      let name = this.getWrapperName() || propName;

      let switchName = this.localizeValueListName(name, itemDesc, i);
      switchName = this.getUniqueLocalizedName(switchName, usedSwitchNames, i + 1);
  ...
  getPropValueRangePercentage() {
    if (this._isValueRangePercentage()) {
      ...
      const calculatedPercentageValue = this.getDevice().convertPropValueToPercentage(this.getProp());
      return Math.max(calculatedPercentageValue, 1); // minimum 1, if we return 0 for some reason the lightbulb jumps to 100%, same with fan?
    }
  }
